...
def solve_qp_dual(Q_, p_, G_, h_, A_, b_, eps=1e-12, verbose=0, notImprovedLim=3,
                  maxIter=20, solver=QPSolvers.PDIPM_BATCHED, check_Q_spd=True):
    """
    Solve a batch of QPs and return primal and dual variables.

    Returns:
        zhats: (nBatch, nz) primal solution
        lams: (nBatch, nineq) inequality duals
        nus: (nBatch, neq) equality duals (empty if no equality)
        slacks: (nBatch, nineq) inequality slacks (h - Gx)
    """
    nBatch = extract_nBatch(Q_, p_, G_, h_, A_, b_)
    Q, _ = expandParam(Q_, nBatch, 3)
    p, _ = expandParam(p_, nBatch, 2)
